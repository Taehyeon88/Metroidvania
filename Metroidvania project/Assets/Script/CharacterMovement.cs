...
public class CharacterMovement : MonoBehaviour
{
    ...
    [Range(0, 0.3f)]
    [SerializeField] private float m_MovementSmoothing = 0.05f; //이동을 부드럽게 하기 위한 평탄화
    ...
    private void Awake()              //이벤트가 추가 안되어 있을 때 이벤트를 할당한다.
    {
    ...
    private void Flip()                  //플립 함수르 호출 하여 방향 전환
    {
    ...
    {
        bool wasGround = m_IsGrounded;       //땅 체크 값을 이전 변수에 넣어서 매 프레임마다 갱신
        m_IsGrounded = false;
        ...
        {
            if (WallColiders[i].gameObject != gameObject)
            {
                m_IsWall = true;
                if (!m_IsGrounded && !status.isDashing && m_Rigidbody2D.velocity.y < 0)
                {
                    m_IsWallSlidding = true;
                    if (!wasWallSlidding)            //이전 프레임이 벽이 아니였다면 체크하여 들려서 벽을 슬라이딩 하고 검출하게 한다
                    {
        ...
        preVelocityX = m_Rigidbody2D.velocity.x;     //이전 속도를 할당한다.
        ...
        if (limitVelOnWallJump)
        {
            ...
            else if (jumpWallDistX < -11f && jumpWallDistX >= -2f)
            {
                status.canMove = true;
        ...
        animator.SetFloat("Speed", Mathf.Abs(m_Rigidbody2D.velocity.x));  //애니메이션 속도 전달
    }
    ...
    public void Move(float move, bool jump, bool dash)
    {
        ...
        {
            if (dash && status.canMove && !m_IsWallSlidding) //대시가 가능한 조건을 검사하여 대시 실행
            {
                status.StartDash(0.1f);
            }
            if (status.isDashing)  //대시 중일 처리
            {
                m_Rigidbody2D.velocity = new Vector2(transform.localScale.x * status.mDashForce, 0);
            }
            else if (m_IsGrounded || m_AirControl)   //플레이어가 땅에 닿았거나 고중 제어가 켜져 있을때
            {
            ...
            {
                animator.SetBool("IsJumping", true);                  //애니메이션 점프 상태 전달
                animator.SetBool("JumpUp", true);                   
                m_IsGrounded = false;                                 //점프하면서 땅에 있지 않으므로 Bool 값 변경
                m_Rigidbody2D.AddForce(new Vector3(0f, m_JumpForce)); //플레이어에게 수직 힘을 추가
                canDoubleJump = true;
            }
            else if (!m_IsGrounded && jump && canDoubleJump && !m_IsWallSlidding)
            {
                ...
                animator.SetBool("IsDoubleJumping", true);                   //애니메이션 더블 점프 상태 전달
            }
