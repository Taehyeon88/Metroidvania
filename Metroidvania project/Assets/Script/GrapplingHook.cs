...
public class GrapplingHook : MonoBehaviour
{
    ...
    private Vector2 hookPosition;               //푹 현재 위치
    private bool isHookTraveling = false;       //훅이 날라가는 중인지 여부
    ...
    private void Update()
    {
        ...
        if (isHookTraveling)                   //훅이 날아가는 중이면 위치 없데이트
        {
            UpdateHookPosition();
        }
    ...
    void UpdateHookPosition()
    {
        ...
        if (Vector2.Distance(hookPosition, grapplePoint) < 0.01f)
        {
